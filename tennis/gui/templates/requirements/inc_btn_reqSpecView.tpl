...
<div id="control_panel" >
  ...
  {if $gui->grants->req_mgmt == "yes"}
    ...
    {if $tlCfg->req_cfg->child_requirements_mgmt == $smarty.const.ENABLED}
      <input class="btn btn-success" type="button" name="btn_new_req_spec" value="{$labels.btn_new_req_spec}"
             onclick="location='{$req_spec_new_url}'" />  
        {/if}
      <input class="btn btn-warning" type="submit" name="edit_req_spec"  value="{$labels.btn_edit_spec}"
         onclick="doAction.value='edit'"/>
      <input class="btn btn-danger" type="button" name="deleteSRS" value="{$labels.btn_delete_spec}"
           onclick="delete_confirmation({$gui->req_spec.id},'{$gui->req_spec.title|escape:'javascript'|escape}',
                                        '{$del_msgbox_title}','{$warning_msg}');"  />

      <input class="btn btn-info" type="button" name="importReqSpec" value="{$btnImportReqSpec}"
             onclick="location='{$req_spec_import_url}'" />
      <input class="btn btn-secondary" type="button" name="exportReq" value="{$labels.btn_export_req_spec}"
             onclick="location='{$req_spec_export_url}'" />
      <input class="btn btn-dark" type="button" name="freeze_req_spec" value="{$labels.btn_freeze_req_spec}"
             onclick="delete_confirmation({$gui->req_spec.id},'{$gui->req_spec.title|escape:'javascript'|escape}',
                   '{$freeze_msgbox_title|escape:'javascript'|escape}', '{$freeze_warning_msg|escape:'javascript'|escape}',
                   pF_freeze_req_spec);"  />
      <input class="btn btn-primary" type="button" name="new_revision" id="new_revision" value="{$labels.btn_new_revision}"
  ...
  {if $gui->revCount > 1}
    <form method="post" 
      ...
      <input class="btn btn-light" type="submit" name="compare_versions" value="{$labels.btn_view_history}" />
    </form>
  {/if}
  ...
  <form class="form-inline" id="req_operations" name="req_operations">
    <fieldset class="text-center center-block">
      <h3>{$labels.req_operations}</h3>
      <hr>
      {if $gui->grants->req_mgmt == "yes"}
        <input class="btn btn-outline-success" type="button" name="create_req" value="{$labels.btn_req_create}"
               onclick="location='{$req_edit_url}'" />  
        <input class="btn btn-outline-info" type="button" name="importReq" value="{$labels.btn_import_reqs}"
               onclick="location='{$req_import_url}'" />
        <input class="btn btn-outline-secondary" type="button" name="exportReq" value="{$labels.btn_export_reqs}"
               onclick="location='{$req_export_url}'" />

        <input class="btn btn-outline-dark" type="button" name="createReqFromIssueXML" value="{$labels.btn_create_from_issue_xml}"
               onclick="location='{$req_create_from_issue_xml_url}'" />

        {if $gui->requirements_count > 0}
          <input class="btn btn-outline-primary" type="button" name="create_tcases"
                 value="{$labels.req_select_create_tc}" onclick="location='{$req_create_tc_url}'" />
        
          <input class="btn btn-outline-warning" type="button" name="copy_req" value="{$labels.btn_copy_requirements}"
                 onclick="location='{$req_spec_copy_req_url}'" />

          <input class="btn btn-outline-default" type="button" name="bulk_mon" value="{$labels.btn_bulk_mon}"
