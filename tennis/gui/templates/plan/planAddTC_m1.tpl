...
<section class="jumbotron">
    <h2 class="text-center">{$gui->pageTitle|escape}{$tlCfg->gui->title_separator_2}{$gui->actionTitle}
        {include file="inc_help.tpl" helptopic="hlp_planAddTC" show_help_icon=true}
    </h2>
</section>
<form name="addTcForm" id="addTcForm" method="post"
	  onSubmit="return checkDelete(js_remove_executed_counter);">
	...
	<div class="container" id="header-wrap">
        {if $gui->has_tc}
            ...
            {if $gui->build.count}
				...
				<div class="well">
                    <div class="col-12 form-inline">
                        <label class="col-5" for="testerID">{$labels.tester_assignment_on_add}</label>
                        <select class="col-lg-2 col-md-2 col-xs-12 col-sm-12 form-control" name="testerID"
                                id="testerID">
                            {html_options options=$gui->testers selected=$gui->testerID}
                        </select>
                    </div>
                    <div class="col-12 form-inline">
                        <label for="build_id" class="col-5">{$labels.build_to_assign_on_add}</label>
					<select class="col-lg-2 col-md-2 col-xs-12 col-sm-12 form-control" id="build_id" name="build_id">
			...
			<div class="">
                    ...
                    <label for="select_platform">{$labels.check_uncheck_all_tc}</label>
                    {if $gui->usePlatforms}
                    ...
                    {$labels.for}
                    {if $gui->full_control}
						<button class="btn btn-primary" onclick="cs_all_checkbox_in_div_with_platform('addTcForm', '{$add_cb}', Ext.get('select_platform').getValue()); return false">{$labels.adding_tc}</button>
                    {/if}
					<button class="btn btn-danger" onclick="cs_all_checkbox_in_div_with_platform('addTcForm', '{$rm_cb}', Ext.get('select_platform').getValue()); return false">{$labels.removal_tc}</button>
                <hr>
                <input type="hidden" name="doAction" id="doAction" value="default" />
				<input class="btn btn-warning" type="submit" name="doAddRemove" value="{$gui->buttonValue}"
					   onclick="doAction.value=this.name" />
                {if $gui->full_control eq 1}
					<input class="btn btn-info" type="submit" name="doReorder" value="{$labels.btn_save_exec_order}"
						   onclick="doAction.value=this.name" />

                    {if $gui->drawSaveCFieldsButton}
						<input class="btn btn-success" type="submit" name="doSaveCustomFields" value="{$labels.btn_save_custom_fields}"
							   onclick="doAction.value=this.name" />
                    {/if}
                    {if $gui->drawSavePlatformsButton}
						<input class="btn btn-dark" type="submit" name="doSavePlatforms" value="{$labels.btn_save_platform}"
							   onclick="doAction.value=this.name" />
                    {/if}
                {/if}
			</div>
        {else}
			<div class="alert alert-info">{$labels.no_testcase_available}</div>
        {/if}
        <br>
    ...
    <hr>
    {if $gui->has_tc}
		<div class="container" id="workback">
            <div class="alert alert-warning">
                {$gui->status_feeback|escape}<br>
                {if $gui->keywords_filter_feedback != '' }
                    <br />{$labels.note_keyword_filter}: <p>{$gui->keywords_filter_feedback|escape}</p>
                {/if}
            </div>
            <br>
            {* ======================================== *}
            ...
            {foreach name="tSuiteLoop" from=$gui->items item=ts}
                ...
                {strip}
					...
					<h2>{$ts.testsuite.name|escape}</h2>
                    {if $item_number == 1}
						<hr />
                    {/if} {* $item_number == 1 *}
                    ...
                    {if ($gui->full_control && $ts.testcase_qty gt 0) || $ts.linked_testcase_qty gt 0}
						<table class="table table-hover">
                            ...
                            <tr>
                                ...
                                {if $gui->usePlatforms} <th>{$labels.th_platform}</th> {/if}
                                <th>{$labels.th_test_case}</th>
                                <th>{$labels.version}</th>
                                <th>{$labels.th_status}</th>
                                {if $gui->priorityEnabled} <th>{$labels.importance}</th> {/if}
                                <th>
