...
<div class="text-center center-block" id="tcView_viewer_tcase_control_panel">
    <form style="display: inline;" id="topControls" name="topControls" method="post" action="{$basehref}lib/testcases/tcEdit.php">
        ...
        {if $edit_enabled && $args_testcase.is_open}
            <input class="btn btn-warning" type="submit" name="edit_tc"
        ...
        {if $delete_enabled && $args_can_do->delete_testcase == "yes" &&  $args_can_delete_testcase == "yes"}
            <input class="btn btn-danger" type="submit" name="delete_tc" value="{$tcView_viewer_labels.btn_delete}" />
        {/if}
        ...
        *}
        {if $args_can_do->copy == "yes" && $args_can_move_copy == "yes"}
            <input class="btn btn-secondary" type="submit" name="move_copy_tc"   value="{$tcView_viewer_labels.btn_mv_cp}" />
        {/if}

        {if $edit_enabled}
            <input type="hidden" name="containerID" value="{$args_testcase.testsuite_id}" />
            <input class="btn btn-success" type="submit" name="new_tc" title="{$tcView_viewer_labels.hint_new_sibling}"
  ...
  <form style="display: inline;" id="tcexport" name="tcexport" method="post" action="{$exportTestCaseAction}" >
    <input type="hidden" name="testcase_id" value="{$args_testcase.testcase_id}" />
    <input type="hidden" name="tcversion_id" value="{$args_testcase.id}" />
    <input class="btn btn-info" type="submit" name="export_tc" value="{$tcView_viewer_labels.btn_export}" />
  </form>



  <form style="display: inline;" id="tcprint" name="tcprint" method="post" action="" >
    <input class="btn btn-primary" type="button" name="tcPrinterFriendly" value="{$tcView_viewer_labels.btn_print_view}"
           onclick="openPrintPreview('tc',{$args_testcase.testcase_id},{$args_testcase.id},null,
                   '{$printTestCaseAction}');"/>
  </form>
    <hr>
    ...
    <form style="display: inline;" id="versionControls" name="versionControls" method="post" action="{$basehref}lib/testcases/tcEdit.php">
        ...
        {if $args_can_do->create_new_version == "yes"}
            <input class="btn btn-outline-success" type="submit" name="do_create_new_version" title="{$tcView_viewer_labels.hint_new_version}"
                   value="{$tcView_viewer_labels.btn_new_version}" />
        {/if}

        {if $delete_enabled && $args_can_do->delete_version == "yes" && $args_can_delete_version == "yes"}
            <input class="btn btn-outline-danger" type="submit" name="delete_tc_version" value="{$tcView_viewer_labels.btn_del_this_version}" />
        {/if}
        ...
        {if $active_status_op_enabled eq 1 && $args_can_do->deactivate=='yes'}
            ...
            {/if}
            <input class="btn btn-outline-danger" type="submit" name="{$act_deact_btn}"
        ...
        $args_can_do->freeze=='yes'}
            ...
            <input class="btn btn-outline-primary" type="submit" name="{$freeze_btn}"
  ...
  <form style="display: inline;" id="addToTestPlans" name="addToTestPlans" method="post" action="">
    <input type="hidden" name="testcase_id" id="versionControls_testcase_id" value="{$args_testcase.testcase_id}" />
    <input type="hidden" name="tcversion_id" value="{$args_testcase.id}" />
    <input class="btn btn-outline-dark" type="button" id="addTc2Tplan_{$args_testcase.id}"  name="addTc2Tplan_{$args_testcase.id}"
  ...
  {* compare versions *}
      ...
      <input type="hidden" name="testcase_id" value="{$args_testcase.testcase_id}" />
      <input class="btn btn-outline-secondary" type="submit" name="compare_versions" value="{$tcView_viewer_labels.btn_compare_versions}" />
    </form>
        {/if}

    <span>
    <input class="btn btn-outline-secondary" type="button" onclick="openExecHistoryWindow({$args_testcase.testcase_id},1);"
        ...
        <input class="btn btn-outline-warning" type="submit" name="bulk_op" value="{$tcView_viewer_labels.btn_bulk}" />
      </form>
        {/if}
...
{if $args_testcase.active eq 0}
    <div class="alert alert-primary" role="alert">
        {$tcView_viewer_labels.tcversion_is_inactive_msg}
    </div>
{/if}
...
<form id="stepsControls" name="stepsControls" method="post" action="{$basehref}lib/testcases/tcEdit.php">
    ...
    {if $edit_enabled && $args_testcase.is_open}
        <div {$addInfoDivStyle}>
            <input class="btn btn-primary" type="submit" name="create_step"
                   onclick="doAction.value='createStep';{$gui->submitCode}" value="{$tcView_viewer_labels.btn_create_step}" />

            {if $args_testcase.steps != ''}
                <input class="btn btn-light" type="submit" name="resequence_steps" id="resequence_steps"
  ...
  <input type="submit" name="renumber_step"
         onclick="doAction.value='doReorderSteps';{$gui->submitCode}; return validateStepsReorder('step_number{$args_testcase.id}');"
                ...
                <td colspan="{$tableColspan}" style="vertical-align:text-top;"><span><a title="{$tcView_viewer_labels.requirement_spec}" href="{$hrefReqSpecMgmt}"
                        ...
                        {if $gui->requirement_mgmt}
                            <img class="clickable" src="{$tlImages.item_link}"
                                 onclick="openReqWindow({$args_testcase.testcase_id},'a');"
                ...
                <td>
                    {section name=item loop=$args_reqs}
                        <img class="clickable" src="{$tlImages.edit}"
                             onclick="openLinkedReqWindow({$args_reqs[item].id});"
