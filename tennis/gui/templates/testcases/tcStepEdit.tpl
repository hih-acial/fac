...
<section class="jumbotron">
  <h2 class="text-center">{$gui->main_descr}</h2>
</section>
...
<form method="post" action="{$basehref}lib/testcases/tcEdit.php" name="tcStepEdit"
  ...
  <div class="groupBtn">
    <input class="btn btn-outline-primary" id="do_update_step" type="submit" name="do_update_step"
           onclick="show_modified_warning=false; doAction.value='{$gui->operation}'"
           value="{$labels.btn_save}" />

    <input class="btn btn-outline-success" id="do_update_step_and_exit" type="submit" name="do_update_step_and_exit"
    ...
    {if $gui->operation == 'doUpdateStep'}
      <input class="btn btn-outline-danger" id="do_create_step" type="submit" name="do_create_step"
             onclick="doAction.value='createStep'" value="{$labels.btn_create_step}" />

      <input class="btn btn-outline-dark" id="do_copy_step" type="submit" name="do_copy_step"
             onclick="doAction.value='doCopyStep'" value="{$labels.btn_copy_step}" />
    {/if}

    <input class="btn btn-outline-secondary" type="button" name="cancel" value="{$labels.btn_cancel}"
           {if $gui->goback_url != ''}  onclick="show_modified_warning=false; location='{$gui->goback_url}';"
           {else}  onclick="show_modified_warning=false; history.back();" {/if} />
  </div>
    <br>
  <table class="table">
  ...
  {if $gui->tcaseSteps != ''}
    ...
    {foreach from=$gui->tcaseSteps item=step_info}
      ...
      <div class="groupBtn">
        ...
        <input  class="btn btn-outline-success" id="do_update_step_and_exit" type="submit" name="do_update_stepand_exit"
               onclick="show_modified_warning=false; doAction.value='{$gui->operation}AndExit'"
               value="{$labels.btn_save_and_exit}" />
