    ...
    <form method="post" id="execSetResults" name="execSetResults"
        ...
        {if !($cfg->exec_cfg->show_testsuite_contents && $gui->can_use_bulk_op)}
            <div class="groupBtn">
                ...
                <input class="btn btn-primary" type="button" name="print" id="print" value="{$labels.btn_print}" onclick="window.print();" />
                <input class="btn btn-warning" type="button" id="toggle_history_on_off"  name="{$gui->history_status_btn_name}"
                       value="{lang_get s=$gui->history_status_btn_name}"
                       onclick="toogleRequiredOnShowHide('bug_summary');
                      toogleRequiredOnShowHide('artifactVersion');
                      toogleRequiredOnShowHide('artifactComponent');
                      execSetResults.submit();"/>

                {if $gui->grants->execute}
                    <input class="btn btn-info" type="button" id="pop_up_import_button" name="import_xml_button"
                           value="{$labels.import_xml_results}"
                           onclick="openImportResult('import_xml_results',{$gui->tproject_id},
                           {$gui->tplan_id},{$gui->build_id},{$gui->platform_id});" />

                {/if}

                {if $tlCfg->exec_cfg->enable_test_automation}
                    <input class="btn btn-dark" type="submit" id="execute_cases" name="execute_cases"
        ...
        show_hide_container_class='jumbotron'
        show_hide_container_view_status_id=$memstatus_id}

        <div id="{$div_id}" class="jumbotron">
        ...
        <div id="{$div_id}" class="jumbotron">
            ...
            {if $gui->build_cfields != ''} <div id="cfields_build" class="jumbotron">{$gui->build_cfields}</div>{/if}
        </div>
        ...
        {else}
            {if $gui->grants->execute == 1 and $gui->build_is_open == 1}
                ...
                {if $cfg->exec_cfg->show_testsuite_contents && $gui->can_use_bulk_op}
                    ...
                    <div id="{$div_id}" name="{$div_id}" class="jumbotron">
        ...
        <div>
            <br />
            <input class="btn btn-secondary" type="button" id="do_export_testcases" name="do_export_testcases"  value="{$labels.btn_export_testcases}"
                   onclick="openExportTestCases('export_testcases',{$gui->node_id},{$gui->tproject_id},
                   {$gui->tplan_id},{$gui->build_id},{$gui->platform_id},
                           '{$gui->tcversionSet}');" />
            <hr>
                        ...
                        <tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
                            ...
                            <td>
                                <img class="clickable" src="{$tlImages.history_small}"
                                     onclick="openExecHistoryWindow({$tc_exec.testcase_id});"
                                     title="{$labels.execution_history}" />
                                <img class="clickable" src="{$tlImages.exec_icon}"
                                     onclick="openExecutionWindow({$tc_exec.testcase_id},{$tcversion_id},{$gui->build_id},{$gui->tplan_id},{$gui->platform_id});"
                                     title="{$labels.execution}" />
                                <img class="clickable" src="{$tlImages.edit}"
                                     onclick="openTCaseWindow({$tc_exec.testcase_id},{$tc_exec.id});"
                                     title="{$labels.design}" />
