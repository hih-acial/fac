...
{foreach item=tc_exec from=$gui->map_last_exec}
    ...
    *}
	...
	<div id="{$div_id}" name="{$div_id}" class="well">
		<div class="exec_testsuite_details" style="width:95%;">
			<span class="text-info">{$labels.details}</span><br />
    ...
    <div class="exec_tc_title">
        <h4 class="text-center">
            ...
            {if $gui->grants->edit_testcase}
               <small> <a href="javascript:openTCaseWindow({$tc_exec.testcase_id},{$tc_exec.id},'editOnExec')">
                    <img src="{$tlImages.note_edit}"  title="{$labels.show_tcase_spec}"> {$labels.show_tcase_spec}
                </a>
               </small>
            {/if}
        </h4>
    ...
    <div id="execution_history" class="jumbotron">
                    ...
                    {if $cfg->exec_cfg->show_last_exec_any_build && $gui->history_on == 0}
                        {if $abs_last_exec.status != '' and $abs_last_exec.status != $tlCfg->results.status_code.not_run}
                            ...
                            <div class="{$tlCfg->results.code_status.$status_code}">
                                ...
                                {/if}
                                {$title_sep_type3}
                                {$labels.build}{$title_sep} {$abs_last_exec.build_name|escape} <br>
                                {$title_sep_type3}
                                {$labels.exec_status} {$title_sep} {localize_tc_status s=$status_code} <br>
                            </div>
        ...
        {if $gui->other_execs.$tcversion_id}
            ...
            <h3>Table des exécutions</h3>
            <hr>
			<table class="table">
                <thead class="thead-dark">
                <tr>
                    <th>{$labels.date_time_run}</th>
                    ...
                    {/if}
                    <th >{$labels.test_exec_by}</th>
                    <th>{$labels.exec_status}</th>
                    <th title="{$labels.execution_duration}">{$labels.execution_duration_short}</th>
                    <th title="{$labels.testcaseversion}">{$labels.version}</th>


                    <th>{$labels.run_mode}</th>
                       ...
                       *}
                        ...
                        <td>
                            ...
                            {if $can_edit_exec_notes && $tc_old_exec.build_is_open}
                                <a href="javascript:openExecEditWindow({$tc_old_exec.execution_id},{$tc_old_exec.id},{$gui->tplan_id},{$gui->tproject_id});" title="{$labels.edit_execution}"><i class="fa fa-edit text-info"> {$labels.edit_execution}</i></a>
                                <br>
                            {else}
                            ...
                            {if ($attachment_model->show_upload_column && !$att_download_only && $tc_old_exec.build_is_open && $can_manage_attachments) || ($attachment_model->show_upload_column && $gui->history_on == 1 && $tc_old_exec.build_is_open && $can_manage_attachments)}
                            <a href="javascript:openFileUploadWindow({$tc_old_exec.execution_id},'executions')">
                                <i class="fa fa-folder-open text-primary" > {$labels.alt_attachment_mgmt}</i></a>
                                <br>
                            {else}
                        ...
                        {if $gui->issueTrackerIntegrationOn}
                                {if $tc_old_exec.build_is_open}
                                    <a href="javascript:open_bug_add_window({$gui->tproject_id},{$gui->tplan_id},{$tc_old_exec.id},{$tc_old_exec.execution_id},0,'link')">
                                        <i class="fa fa-bug text-danger"> {$labels.bug_link_tl_to_bts}</i></a>
                                    <br>
                                    {if $gui->tlCanCreateIssue}
                                        <a href="javascript:open_bug_add_window({$gui->tproject_id},{$gui->tplan_id},{$tc_old_exec.id},{$tc_old_exec.execution_id},0,'create')">
                                            <i class="fa fa-plus text-success"> {$labels.bug_create_into_bts}</i> </a>
                                        <br>
                                    {/if}
                                {else}
                                    <img src="{$tlImages.bug_link_tl_to_bts_disabled}" title="{$labels.bug_link_tl_to_bts}" style="border:none" />
                                    <br>
                                    {if $gui->tlCanCreateIssue}
                                        <img src="{$tlImages.bug_create_into_bts_disabled}" title="{$labels.bug_create_into_bts}" style="border:none" />
                                        <br>
                                    {/if}
                                {/if}
                        ...
                        {if $can_delete_exec && $tc_old_exec.build_is_open}
                                ...
                                <a href="javascript:confirm_and_submit(msg,'execSetResults','exec_to_delete',{$tc_old_exec.execution_id},'do_delete',1);">
                                    <i class="fa fa-close text-danger"> {$labels.img_title_delete_execution}</i></a>
                            <br>
                        {else}
                            {if $can_delete_exec}
                        ...
                        {* CORTADO *}
                        {if $tlCfg->exec_cfg->steps_exec }
                            <a href="javascript:openPrintPreview('exec',{$tc_old_exec.execution_id}, null,null,'{$printExecutionAction}');"><i class="fa fa-building text-dark"> {$labels.access_test_steps_exec}</i></a>
                            <br>
                        {/if}
                        </td>
