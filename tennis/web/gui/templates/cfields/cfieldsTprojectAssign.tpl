...
{if $gui->linkedCF ne ""}
  <section class="container container-fluid">
    <h2 class="well text-center text-success">{$labels.title_assigned_cfields}</h2>
	  <hr>
    <form class="form-horizontal" method="post">
      ...
      <table class="table table-striped table-hover">
		  ...
		  <tr>
			  <th align="center"  style="width: 5px;background-color:#00AA00;">
        ...
        {foreach key=cf_id item=cf from=$gui->linkedCF}
			<tr>
				...
				<td >
					<input type="checkbox" name="active_cfield[{$cf.id}]"
        ...
        <input type="hidden" name="doAction" value="" />
    		...
    		<input class="btn btn-danger" type="submit" name="doUnassign" value="{$labels.btn_unassign}"
    		                     onclick="doAction.value=this.name"/>
    		                     
    		<input class="btn btn-success" type="submit" name="doBooleanMgmt" value="{$labels.btn_cfields_boolean_mgmt}"
    		                     onclick="doAction.value=this.name"/>

    		<input class="btn btn-warning" type="submit" name="doReorder" value="{$labels.btn_cfields_display_attr}"
    		                     onclick="doAction.value=this.name"/>
...
{if $gui->other_cf ne ""}
  <section class="container container-fluid">
    <h2 class="well text-center text-success">{$labels.title_available_cfields}</h2>
	  <hr>
    <form method="post">
      <div id="free_cf"> 
 	    {* used as memory for the check/uncheck all checkbox javascript logic *}
      ...
      <table class="table table-striped table-hover" >
    	...
    	<div class="center-block text-center">
        <input type="hidden" name="doAction" value="" />
    		<input class="btn btn-success" type="submit" name="doAssign" id=this.name value="{$labels.btn_assign}"
    		                     onclick="doAction.value=this.name"/>
    	</div>
    </form>
    </section>
