...
<form id='tc_exec_assignment' name='tc_exec_assignment' method='post'>
  ...
  {if $gui->has_tc}
    ...
    <hr>
        <div>
            <div class="form-group">
                <label for="bulk_tester_div"><img src="{$tlImages.user_group}" title="{$labels.user_bulk_assignment}">
                    {$labels.user_bulk_assignment}</label>
            ...
            <div class="form-group">
                <label for="{$labels.send_mail_to_tester}">{$labels.send_mail_to_tester}</label>
                <input id="{$labels.send_mail_to_tester}" type="checkbox" title="{$labels.send_mail_to_tester}"
                       name="send_mail" id="send_mail" {$gui->send_mail_checked} />
            </div>
            <hr>
			<input class="btn btn-success" type='button' name='bulk_user_assignment' id='bulk_user_assignment'
				onclick='if(check_action_precondition("tc_exec_assignment","default"))
						        setComboIfCbx("tc_exec_assignment_cb","tester_for_tcid_",
                                  "bulk_tester_div")'
				value="{$labels.btn_do}" />
			<input class="btn btn-info" type='submit' name='doActionButton' id='doActionButton' value='{$labels.btn_update_selected_tc}' />
    ...
    <div>
      <input class="btn btn-outline-success" type='submit' name='doBulkUserRemove' id='doBulkUserRemove' value='{$labels.user_bulk_remove}' />
      <input class="btn btn-outline-dark" type='button' name='linkByMail'
             id='linkByMail'
      ...
      <hr>
  {else}
	  <div class="alert alert-warning">{$labels.no_testcase_available}</div>
  {/if}
	</div> <!-- header-wrap -->

  {if $gui->has_tc}
   <div class="container" id="tc_exec_assignment_cb">
	  ...
	  {foreach from=$gui->items item=ts key=idx name="div_drawing"}
        ...
        <input type="hidden" name="add_value_{$ts_id}"  id="add_value_{$ts_id}"  value="0" />
          ...
          {if $ts.testcase_qty gt 0}
            ...
            <table cellspacing="0" style="font-size:small;" width="100%" id="the-table-{$table_counter}" class="table table-hover">
            {* ---------------------------------------------------------------------------------------------------- *}
			      {* Heading *}
