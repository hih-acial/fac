...
<h1 class="text-success text-center">{$gui->main_title}</h1>
...
{if $gui->features neq ''}
    <div class="panel panel-success">
        ...
        <form class="form-horizontal" method="post" action="{$umgmt}/usersAssign.php"
	{if $tlCfg->demoMode}
	...
	<input type="hidden" name="featureType" value="{$gui->featureType}" />
            ...
            <label for="featureSel" class="control-label col-lg-2">{$labels.TestProject}{$gui->accessTypeImg}</label>
            {else}
                <label for="featureSel" class="control-label col-lg-2">{$labels.TestProject}{$gui->tprojectAccessTypeImg} &nbsp; {$gui->tproject_name|escape}</label>
            {/if}
            <select class="form-control" id="featureSel" onchange="changeFeature('{$gui->featureType}')">
            ...
            <div class="form-group form-inline">
                <label for="allUsersRole" class="control-label col-lg-2">{$labels.set_roles_to}</label>
                <select class="form-control" name="allUsersRole" id="allUsersRole">
                ...
                <input type="button" class="btn btn-success" value="{$labels.btn_do}"
                       onclick="set_combo_group('usersRoleTable','userRole_',
    ...
    <div id="usersRoleTable">
	    <table class="table table-bordered table-hover sortable" width="100%" id="item_view">
            ...
            <tr>
                <th>
                    {$tlImages.sort_hint}{$labels.User}
                </th>
                {assign var="featureVerbose" value=$gui->featureType}
                <th>
                    {$tlImages.sort_hint}{lang_get s="th_roles_$featureVerbose"} ({$my_feature_name|escape})
                </th>
            </tr>
            </thead>
    	    <tbody>
    	...
    	<tr {$user_row_class} bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
    		...
    		<td>
                <label >
          <select class="form-control" name="userRole[{$uID}]" id="userRole_{$uID}"
